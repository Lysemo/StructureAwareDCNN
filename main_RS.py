...
def train():
    ...
    criterion = JointMseLoss().to(device)
    best_loss = np.inf
    best_psnr = -1
    for epoch in range(args['epochs']):
        ...
        model.train()
        train_loss = []
        for i, (image, gt, sobel_gt) in enumerate(trainDataLoader):
            image, gt, sobel_gt = image.to(device), gt.to(device), sobel_gt.to(device)
            pre1,pre2 = model(image)
            pre1 = torch.tanh(pre1)
            pre2 = torch.tanh(pre2)
            loss = criterion(pre1,pre2,gt,sobel_gt)
        ...
        PSNR = []
        with torch.no_grad():
            for j,(image,gt,sobel_gt) in tqdm(enumerate(valDataLoader),total=len(valDataLoader)):
                image,gt,sobel_gt = image.to(device),gt.to(device), sobel_gt.to(device)
                pre1, pre2 = model(image)
                pre1 = torch.tanh(pre1)
                pre2 = torch.tanh(pre2)
